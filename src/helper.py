# for i in $(objdump -d bin/impure|grep "first_mutation" -A 90 |grep "^ " |cut -f2); do echo -n '\x'$i; done; echo
import sys


mem = "" + \
    "" + \
    "" + \
    "" + \
    "" + \
    "" + \
    "" + \
    ""

first_func = "" 

second_func = ""

third_func = ""

def convert_to_c_array(byte_str):
    print "Length : {}".format(len(byte_str))
    print "{"
    array_str = ["0x{:02x}".format(int(ord(b))) for b in byte_str]
    for i in range(len(array_str)):
        sys.stdout.write(array_str[i])
        if (i+1) != len(byte_str): sys.stdout.write(", ")
        if ((i + 1) % 16) == 0:
            print ''
    print ""
    print "}"

# the offset is in mem2
def xor_mem(mem1, mem2, offset):
    return ''.join([chr(ord(mem1[i]) ^ ord(mem2[offset + i])) for i in range(len(mem1))])


def xor_flag(flag):
    xoring = [0xd9, 0xd4, 0xc8]
    a = ''.join([chr(ord(c) ^ xoring[i % len(xoring)]) for i, c in enumerate(flag)])
    print repr(a)
    return a


def shift_flag(flag):
    return [hex(ord(flag[i]) << 6) for i in range(len(flag))]

second_flag_part = "3ck_d1d_y0u_l34r"
third_flag_part = "n_Pr0Gr4MM1nG}"

# convert_to_c_array(xor_flag(second_flag_part))
# convert_to_c_array(xor_mem(second_func, first_func, 0))

# print shift_flag(third_flag_part)

convert_to_c_array(xor_mem(first_func, mem, 0))
convert_to_c_array(xor_mem(second_func, first_func, 0x10))
convert_to_c_array(xor_mem(third_func, mem, 0x120))
print len(mem)

